...
<script>
  ...
  data: {
	...
	$('#table > table').append(
          ...
          '<td class="tabletext" width="15%";>' + tel + '</td>' +
          '<td class="tabletext" width="28%";>' + price + 
          '<a href="reservation?res_title='+encodeURIComponent(title)+'&&res_addr='+encodeURIComponent(addr1)+
        		  '&&res_price='+encodeURIComponent(price)+'&&res_img_addr='+encodeURIComponent(firstImage)+'"><button type="button" class="btn btn-primary" >예약</button></a></td>'+
  ...
  function generateRandomPrice() {
	    ...
	    // 숫자를 통화 형식(3자리마다 쉼표)으로 변환하여 반환
	    return new Intl.NumberFormat('ko-KR').format(randomPrice) + '원';
	}
